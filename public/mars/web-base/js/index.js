/* 2017-12-7 10:33:50 | 修改 木遥（QQ：346819890） */
//loading bar
...
// liangfan added 初始化时 外部调用
function initMarsBase() {
...
function initMap() {
    ...
    var configfile = "/mars/web-base/config.json";
    if (request.config)
    ...
    mars3d.createMap({
        ...
        success: function (_viewer, gisdata, jsondata) {//地图成功加载完成后执行 
            ...
            //     // viewer.mars.centerAt(request, { duration: 0, isWgs84: true });// liangfan ignored 禁止定位
            // }
            ...
            //     initWork(_viewer);
            // }
            // hasAnimation = false;// liangfan add 取消开场动画
...
function limitCamera() {
    ...
    handler.setInputAction(function (e) {
        var cartesian = mars3d.latlng.getCurrentMousePosition(viewer.scene, e.position);
        if (cartesian) {
...
function bindToLayerControl(options) {
    ...
    var manageLayersWidget = mars3d.widget.getClass('assets/mars/widgets/manageLayers/widget.js');
...
function activateWidget(item) {
    console.log("activateWidget", item);
...
function disableWidget(item){
    mars3d.widget.disable(item.uri);
    // mars3d.widget.dis
}
mars3d.initMarsBase=initMarsBase;
mars3d.activateWidget=activateWidget;
mars3d.disableWidget=disableWidget;
// liangfan added export 控件初始化方法
// export { initMarsBase ,activateWidget, disableWidget}
