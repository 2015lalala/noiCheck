...
var tableWork = {
    ...
    initEvent: function () {
        ...
        $("#btnFlyStart").click(function (e) {
            ...
            that.loadData(thisWidget.arrFlyTable);
        });
    ...
    updateLatlngsHtml: function (latlngs, cfg) {
        ...
        if (!latlngs || latlngs.length == 0) {
            ...
            inHtml += ' <div class="mp_attr" style=" margin-top: 10px;"><table>'
                + ' <tr> <td class="nametd">经度：</td> <td><input type="number" class="mp_input plot_latlngs" data-type="jd" min=-180 max=180 value="' + jd + '"></td></tr>'
                + '<tr>  <td class="nametd">纬度：</td> <td><input type="number" class="mp_input plot_latlngs" data-type="wd" min=-90 max=90  value="' + wd + '"></td></tr>';
            if (cfg.height)
            ...
            for (var idx = 0; idx < latlngs.length; idx++) {
                ...
                inHtml += '<div><div class="open"><i class="tree_icon">-</i>第' + (idx + 1) + '点 <label style="width:100px;">&nbsp;</label>    ' + addthml + delhtml + ' </div><div class="mp_attr"><table>'
                    + '<tr> <td class="nametd">经度：</td> <td><input  type="number" class="mp_input plot_latlngs" data-type="jd" min=-180 max=180  data-index="' + idx + '" value="' + jd + '"></td>  </tr> '
                    + '<tr>  <td class="nametd">纬度：</td> <td><input  type="number" class="mp_input plot_latlngs" data-type="wd" min=-90 max=90  data-index="' + idx + '" value="' + wd + '"></td> </tr> ';
                if (cfg.height)
    ...
    getAttrInput: function (parname, attrName, attrVal, edit) {
        ...
        switch (edit.type) {
            ...
            case "number":
                ...
                //     });
                // };
                var minhtml=edit.min>=0?" min="+edit.min:"";
                var maxhtml=edit.max>=0?" max="+edit.max:"";
                 inHtml = '<input id="' + parname + attrName + '" type="number" value="' + attrVal + '" '+minhtml+maxhtml+'    class="mp_input"/>';
                 fun = function (parname, attrName, attrVal, edit) {
                     $('#' + parname + attrName).on("input propertychange", function (e) {
                         var attrVal = Number($(this).val());
                         if((edit.min||edit.min==0)&&attrVal<edit.min){
                             $(this).val(edit.min)
                             attrVal=0;
                         }
                         that.updateAttr(parname, attrName, attrVal);
