...
var fl = function () {
    ...
    this.viewWindow = window,
        //每个窗口创建完成后调用
        this.winCreateOK = function (opt, result) {
    ...
    this.start = function (lineData) {
        ...
        for (var i = 0, length = lonlats.length; i < length; i++) {
            ...
            if (i == 0) {//起点
                ...
                var speed = speeds ? speeds[i - 1] : defSpeed; //1千米/时 
                ...
                var speed = speeds ? speeds[i - 1] : defSpeed; //1千米/时 =  1/3.6 米/秒
                speed = speed * 1;//转弯处降速 
        ...
        switch (attr.point) {
            ...
            case "model_car"://汽车模型   
                entityAttr.model = {
                    uri: '/assets/cesium-models/CesiumGround/Cesium_Ground.glb',
                    scale: 0.2,
                    minimumPixelSize: 50
                };
                if (entityAttr.label)
            ...
            case "model_air"://飞机模型
                entityAttr.model = {
                    uri: '/assets/cesium-models/CesiumAir/Cesium_Air.glb',
                    scale: 0.1,
                    minimumPixelSize: 50
                };
                if (entityAttr.label)
            ...
            case "model_weixin"://卫星模型
                entityAttr.model = {
                    uri: '/assets/mars/web-base/data/gltf/weixin.gltf',
                    scale: 1,
    ...
    this.scratch = new Cesium.Matrix4(),
        this.preRender_eventHandler = function (e) {
        ...
        this.realTime = function (position) {
            var attr = this.flyEntity.data.properties.attr;

            var point = mars3d.latlng.formatPositon(position);
            var time = this.viewer.clock.currentTime.secondsOfDay - this.viewer.clock.startTime.secondsOfDay;//已飞行时间
            ...
            mars3d.util.terrainPolyline({
                ...
                calback: function (raisedPositions, noHeight) {
                    ...
                    var hbgd = mars3d.latlng.formatPositon(raisedPositions[0]).z; //地面高程
        ...
        this.getModelMatrix = function (entity, time, result) {
    ...
    //显示剖面
    this.charsWidgetUri = 'widgets/roamChars/widget.js',
        this.charsCacheData = null,
        this.showHeightChars = function () {
        ...
        this.updateCharsWidge = function (data) {
        ...
        this.updateCharsWidgeFlyOk = function (alllen) {
            var roamingJK = mars3d.widget.getClass(this.charsWidgetUri);
            if (roamingJK && roamingJK.isActivate) {
                roamingJK.changeFlyOk(alllen);
            }
        },
        this.getTerrainHeight = function (calback) {
            var that = this;
                ...
                mars3d.util.terrainPolyline({
                    ...
                    calback: function (raisedPositions, noHeight) {
                        ...
                        for (var i = 0; i < raisedPositions.length; i++) {
                            ...
                            //坐标
                            var point = mars3d.latlng.formatPositon(raisedPositions[i]);
                            arrPoint.push(point);
    ...
    //投影
    this.wallEntity = null,
        this.updateWall = function (positions) {
            var newposition = [];
            var minimumHeights = [];
            var maximumHeights = [];
            ...
            if (this.wallEntity) {
                this.wallEntity.wall.minimumHeights = new mars3d.draw.DynamicProperty(minimumHeights);
                this.wallEntity.wall.maximumHeights = new mars3d.draw.DynamicProperty(maximumHeights);
                this.wallEntity.wall.positions = new mars3d.draw.DynamicProperty(newposition);
            }
            else {
                var wallattr = mars3d.draw.WallControl.attribute2Entity({ "style": { "color": "#d7e600", "outline": false, "opacity": 0.5 } });
                wallattr.minimumHeights = minimumHeights;
                wallattr.maximumHeights = maximumHeights;
                wallattr.positions = new mars3d.draw.DynamicProperty(newposition);
        ...
        //获取已飞行完成的点
        this.getFlyOkPoints = function (position) {
            var arrnew = [];
            var alllen = 0;

            var thistime = this.viewer.clock.currentTime.secondsOfDay;
            var arr = this.arrLinePoint;
            for (var i = 0, length = arr.length; i < length; i++) {
                var item = arr[i];
                if (item.time.secondsOfDay > thistime) {
                    var len = Cesium.Cartesian3.distance(position, arr[i - 1]);
                    alllen += len;
                    break;
                }
                if (i > 0) {
        ...
        //计算转弯
        this.getBezierSpline = function (pt1, pt2, pt3) {
        ...
        this.getPointForLineAlong = function (p1, p2, len, bl) {
        ...
        this.createTimeLine = function () {
            var timeline= parent.dePluginManager.getWidgetByName("时间线");
            if(timeline.state!=0){
                parent.dePluginManager.showWidget(timeline);
                this.shouldCloseTimeLine=true;
            }
    ...
    this.onTimelineScrubfunction = function (e) {
        var clock = e.clock;
    ...
    this.removeTimeLine = function () {
        var timeline= parent.dePluginManager.getWidgetByName("时间线");
        if(this.shouldCloseTimeLine){
            parent.dePluginManager.closeWidget(timeline);
        }
        // var timeline= parent.dePluginManager.getWidgetByName("时间线");
        // if(timeline.state!=0){
        //     parent.dePluginManager.showWidget(timeline);
