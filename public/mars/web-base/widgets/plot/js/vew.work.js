...
var plotFile = {
    initEvent: function () {
        ...
        $("#input_plot_file").change(function (e) {
            ...
            if (fileType != "json") {
                toastr.warning('文件类型不合法,请选择json格式标注文件！');
                that.clearPlotFile();
                return;
            }
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (image) {
                if (image.startsWith("$plot$"))
                    image = image.replace("$plot$/", "/mars/web-base/");  //是模块内部本级图片
                else
                    image = "/mars/web-base/" + image;   //相对于父级index页面的图片
                if(item.style&&item.style.image){
                ...
                inhtml += ' <li onclick="plotlist.startPlot(' + i + ',this)"> <i title="'
                    ...
                    if(item.style&&item.style.modelUrl){
                        item.style.modelUrl="/mars/web-base/"+item.style.modelUrl;
                    }
            }
            else {
                //使用字体图标 
